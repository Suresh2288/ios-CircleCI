...
class VideoCapture: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    fileprivate func setSessionPreset() throws {
        if (session!.canSetSessionPreset(AVCaptureSession.Preset(rawValue: convertFromAVCaptureSessionPreset(AVCaptureSession.Preset.vga640x480)))) {
    ...
    fileprivate func getImageFromBuffer(_ buffer: CMSampleBuffer) -> CIImage {
        ...
        let image = CIImage(cvPixelBuffer: pixelBuffer!, options: convertToOptionalCIImageOptionDictionary(attachments as? [String : AnyObject]))
    ...
    fileprivate func alterPreview(_ features: [CIFeature], cleanAperture: CGRect) {
        ...
        if (features.count == 0 || cleanAperture == CGRect.zero || !isCapturing) {
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "updateDistance"), object: nil)
            return
        }
        ...
        for feature in features {
            ...
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: "updateDistance"), object: faceFeature!.bounds.size.width)
