...
class ChildSettingsVCiPad: _BaseScrollViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(ChildSettingsVCiPad.initChildSetting), name: .reloadSummaryCustomiseSettings, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(ChildSettingsVCiPad.goToPremium), name: .goSignUp, object: nil)
    ...
    @objc func initChildSetting(){
        ...
        viewModel.getCustomiseSettingsSummary(success: { _ in
    ...
    @IBAction func btnBlockScheduleTapped(_ sender : Any){
        if let vc = UIStoryboard.CustomiseSettings() as? CustomiseSettingsVC{
            ...
            if let presented = self.presentedViewController {
                presented.dismiss(animated: false) { () -> Void in
    ...
    func updateRemoteLock(){
        viewModel.updateChildLock(success: { _ in
...
extension ChildSettingsVCiPad : MaterialSwitchDelegate{
    ...
    func switchDidChangeState(currentSwitch: MaterialSwitch, currentState: MaterialSwitchState) {
        ...
        if currentSwitch.tag == 1{
            ...
            self.viewModel.updateBlockBrowser(success: { _ in
            ...
            self.viewModel.updatePosture(success: { _ in
...
extension ChildSettingsVCiPad: CLLocationManagerDelegate {
