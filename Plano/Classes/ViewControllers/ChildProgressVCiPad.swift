...
class ChildProgressVCiPad: _BaseViewController, UIScrollViewDelegate{
    ...
    @IBOutlet weak var lblPlanoEyeInfo: AdaptiveLabel!
    ...
    @IBOutlet weak var lblNextDueDate: AdaptiveLabel!
    @IBOutlet weak var lblLastCheckedDate: AdaptiveLabel!
    @IBOutlet weak var lblDigitalEyeStrain: AdaptiveLabel!
    @IBOutlet weak var lblPlanoPoints: AdaptiveLabel!
    @IBOutlet weak var lbllDigitalEyeBehaviour: AdaptiveLabel!
    ...
    @IBOutlet weak var lblDigitalEyeBehaviourGoal: AdaptiveLabel!
    @IBOutlet weak var lblDigitalEyeStrainGoal: AdaptiveLabel!
    ...
    @IBOutlet weak var btnBookNow: AdaptiveButton! {
        didSet{
    ...
    func initChildProgress(){
        ...
        viewModel.getProgress(success: { _ in
            ...
            self.overallProgressAngle = self.progresstoAngle(progress: childProgressObj.overAllProgress)
            
            if childProgressObj.overAllProgress == "0"{
                self.lblLastMonthScore.isHidden = true
                self.lblLastMonthTotalScore.isHidden = true
                self.lblLastMonthsNoRecordsFound.isHidden = false
                self.lblLastMonthsNoRecordsFound.text = "No Records Found".localized()
            }else{
                self.lblLastMonthScore.isHidden = false
                self.lblLastMonthTotalScore.isHidden = false
                self.lblLastMonthsNoRecordsFound.isHidden = true
                self.lblLastMonthScore.text = childProgressObj.overAllProgress
            }
            
            self.todayProgressAngle = self.progresstoAngle(progress: childProgressObj.todayProgress)
            
            if childProgressObj.todayProgress == "0"{
                self.lblTodayScore.isHidden = true
                self.lblTodayTotalScore.isHidden = true
                self.lblTodayNoRecordsFound.isHidden = false
                self.lblTodayNoRecordsFound.text = "No Records Found".localized()
            }else{
                self.lblTodayScore.isHidden = false
                self.lblTodayTotalScore.isHidden = false
                self.lblTodayNoRecordsFound.isHidden = true
                self.lblTodayScore.text = childProgressObj.todayProgress
            }
            ...
            self.lblLastMonthScore.text = ""
            self.lblTodayScore.text = ""
    ...
    func initMyopiaAndReports(){
        ...
        viewModel.getMyopiaLastRecordsAndReport(success: { 
                ...
                ChildApiManager.sharedInstance.getChildEyeHealth(request, completed: { (apiResponseHandler, error) in
                    ...
                    if apiResponseHandler.isSuccess() {
                        ...
                        if let response = Mapper<ChildEyeHealthResponse>().map(JSONObject: apiResponseHandler.jsonObject) {
                            ...
                            self.lbllDigitalEyeBehaviour.text = String(response.DigitalEyeBehaviour)
...
//
//                self.lbllDigitalEyeBehaviour.text = String(DigitalEyeBehaviour)
                    ...
                    if apiResponseHandler.isSuccess() {
                        ...
                        if let response = Mapper<ChildEyeCheckResponse>().map(JSONObject: apiResponseHandler.jsonObject) {
                            ...
                            if (Int(response.IsShowMessage) == 0) {
                                ...
                                self.MyopiaViewHeightConstraint.constant = 285
                            } else if (Int(response.IsShowMessage) == 1) {
                                self.lblPlanoEyeInfo.isHidden = false
                                self.MyopiaViewHeightConstraint.constant = 310
                            }
...
extension ChildProgressVCiPad : CAPSPageMenuDelegate{
...
extension ChildProgressVCiPad : UICollectionViewDelegate, UICollectionViewDataSource{
...
extension ChildProgressVCiPad : MaterialSwitchDelegate{
