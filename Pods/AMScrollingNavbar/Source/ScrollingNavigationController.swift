...
open class ScrollingNavigationController: UINavigationController, UIGestureRecognizerDelegate {
  ...
  open func followScrollView(_ scrollableView: UIView, delay: Double = 0, scrollSpeedFactor: Double = 1, collapseDirection: NavigationBarCollapseDirection = .scrollDown, additionalOffset: CGFloat = 0, followers: [NavigationBarFollower] = []) {
    ...
    NotificationCenter.default.addObserver(self, selector: #selector(ScrollingNavigationController.willResignActive(_:)), name: .UIApplicationWillResignActive, object: nil)
    NotificationCenter.default.addObserver(self, selector: #selector(ScrollingNavigationController.didBecomeActive(_:)), name: .UIApplicationDidBecomeActive, object: nil)
    NotificationCenter.default.addObserver(self, selector: #selector(ScrollingNavigationController.didRotate(_:)), name: .UIDeviceOrientationDidChange, object: nil)
    NotificationCenter.default.addObserver(self, selector: #selector(ScrollingNavigationController.windowDidBecomeVisible(_:)), name: .UIWindowDidBecomeVisible, object: nil)
  ...
  open func stopFollowingScrollView(showingNavbar: Bool = true) {
    ...
    center.removeObserver(self, name: .UIDeviceOrientationDidChange, object: nil)
  }
  ...
  private func updateNavbarAlpha() {
    ...
    let titleDict: NSDictionary = [NSAttributedStringKey.foregroundColor: UIColor.black.withAlphaComponent(alpha)]
